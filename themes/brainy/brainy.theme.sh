...
function ___brainy_prompt_conda {
    [[ $THEME_SHOW_CONDA != true ]] && return
    local color=$_omb_prompt_bold_olive
    local box="[|]"
    local conda_env="$(echo $CONDA_DEFAULT_ENV)"
    printf "%s|%s|%s|%s" "$color" "${conda_env}" "$_omb_prompt_bold_navy" "$box"
}
...
THEME_SHOW_CONDA=${THEME_SHOW_CONDA:-"true"}
...
___BRAINY_TOP_LEFT=${___BRAINY_TOP_LEFT:-"conda user_info dir scm"}
